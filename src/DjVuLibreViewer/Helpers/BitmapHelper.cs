...
namespace DjVuLibreViewer.Helpers
{
    internal static class BitmapHelper
    {
        ...
        public static Bitmap CreateBitmapFromRgb24(byte[] rgbBytes, int width, int height)
        {
            ...
            // Ensure the stride matches
            if (bmpData.Stride == stride)
            {
                // Fast copy if stride matches
                Marshal.Copy(rgbBytes, 0, bmpData.Scan0, rgbBytes.Length);
            }
            else
            {
                // Copy row by row if stride is padded
                for (int y = 0; y < height; y++)
                {
                    IntPtr dest = bmpData.Scan0 + y * bmpData.Stride;
                    Marshal.Copy(rgbBytes, y * stride, dest, stride);
                }
            }
        ...
        public static BitmapSource CreateFromRgb24(byte[] rgbBytes, int width, int height)
        {
            int bytesPerPixel = 3;
